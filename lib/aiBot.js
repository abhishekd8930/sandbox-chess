...
export function generateAIMove(boardState, aiColor = 'b', difficultyOrBluffRate = 'medium') {
  if (!boardState) return null;

  const rows = boardState.length;
  const cols = boardState[0].length;

  let bluffChance = 0.20;
  if (typeof difficultyOrBluffRate === 'number') {
    bluffChance = difficultyOrBluffRate;
  } else if (difficultyOrBluffRate === 'beginner') {
    bluffChance = 0.35;
  } else if (difficultyOrBluffRate === 'grandmaster') {
    bluffChance = 0.12;
  } else if (difficultyOrBluffRate === 'easy') {
    bluffChance = 0.35;
  } else if (difficultyOrBluffRate === 'hard') {
    bluffChance = 0.12;
  }
